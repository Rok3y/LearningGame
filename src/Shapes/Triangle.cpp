...
{
	// We create 3 sub-triangles and if all of these adds up into thisArea (is equal), then the point is within triangle
	float thisArea = Area();
...
{
	Vec2D currentCenter = GetCenterPoint();
	mPoints[0] += p - currentCenter;
	mPoints[1] += p - currentCenter;
	mPoints[2] += p - currentCenter;
}